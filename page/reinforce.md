...
| 항목        | 기존 DQN                      | Double DQN                     |
| --------- | --------------------------- | ------------------------------ |
| 행동 선택과 평가 | 같은 Q로 선택하고 평가함 → 과장됨        | 선택은 online Q, 평가는 target Q로 나눔 |
| 결과        | 너무 높게 Q값을 믿음 (overestimate) | 더 정확한 Q값 예측                    |
...
#### 7. 일반 Experience Replay와 비교

| 항목                       | 일반 Experience Replay             | Prioritized Experience Replay             |
|----------------------------|-------------------------------------|-------------------------------------------|
| 샘플링 방식                 | 균등 무작위                         | TD-error 기반 확률적 샘플링               |
| 중요한 경험에 대한 처리     | 모두 동일하게 취급                 | TD-error 큰 경험을 자주 학습              |
| 샘플 효율성                 | 낮을 수 있음                        | 높음                                      |
| 학습 수렴 속도              | 느릴 수 있음                        | 빠르게 수렴 가능                          |
| 단점                       | 단순하지만 비효율적                | 구현 복잡, importance sampling 보정 필요  |
...
#### 기존 DQN vs 듀얼링 DQN 비교

| 항목             | 기존 DQN                          | 듀얼링 DQN                                              |
|------------------|-----------------------------------|----------------------------------------------------------|
| Q값 계산 방식     | $$Q(s, a)$$ 직접 예측             | $$Q(s, a) = V(s) + (A(s, a) - \text{mean}(A))$$           |
| 상태 가치 고려    | X                                 | O                                                        |
| 행동 간 차이 반영 | 모든 Q에 포함됨                   | A(s, a)에만 반영됨                                       |
| 예시 해석         | "콜라 Q=8.2, 사이다 Q=8.1 등"     | "상태가 8.0이고, 콜라는 +0.1, 사이다는 0.0, 커피는 -0.1" |
